...
public class TestIPAddress {
    ...
    static XenDispatcher xend = null;
    
    public static void main(String[] args){
        try{
            xend = XenDispatcher.getInstance();
            List<VM.Record> vms = xend.getAllVMRecords();
            String uuid = "";
            for(VM.Record vm : vms){
                if(!vm.isATemplate && vm.nameLabel.contains("Debian Sarge")){
                    uuid = vm.uuid;
                    System.out.println(uuid);
                    break;
                }
            }
            String ipAdd = xend.getIpAddress(uuid);
